...
export async function POST(req) {
  try {
    ...
    console.log("Received formData:", { fileName, mimeType, folderId });
    ...
    if (!folderId) {
      ...
      return NextResponse.json({ success: false, error: "No folder ID provided." });
    }
    ...
    console.log("Creating resumable upload session with metadata:", fileMetadata);
    ...
    if (!uploadUrl) {
      console.error("No uploadUrl returned by Google Drive API");
      return NextResponse.json({ success: false, error: "Failed to generate upload URL" });
    }
