// api/folders/route.js
import { NextResponse } from "next/server";
import { google } from "googleapis";

export async function POST(req) {
  try {
    const { folderName, parentPath, parentFolderId: providedParentFolderId } = await req.json();
    
    if (!folderName) {
      return NextResponse.json({ success: false, error: "Folder name is required" });
    }

    // Hardcoded Google Service Account credentials (same as in your upload route)
    const serviceAccountKey = {
      "type": "service_account",
      "project_id": "rahul-nagar-454910",
      "private_key_id": "1a673f0b897c0f1489166c9c7560f95072093012",
      "private_key": "-----BEGIN PRIVATE KEY-----\\n9gl/oHhgJ869EQq1lxY5IEaG\n-----END PRIVATE KEY-----\n",
      "client_email": "sheets-service-account@rahul-nagar-454910.iam.gserviceaccount.com",
      "token_uri": "https://oauth2.googleapis.com/token"
    };

    // Default root folder ID 
    const rootFolderId = "1cXQEwyr8n2-ydx9fTP7uN1dsudLEXM41";

    // Authenticate with Google Drive API
    const auth = new google.auth.GoogleAuth({
      credentials: serviceAccountKey,
      scopes: ["https://www.googleapis.com/auth/drive"],
    });

    // Initialize Google Drive
    const drive = google.drive({ version: "v3", auth });

    // Use explicitly provided parentFolderId if available, otherwise determine it from path
    let parentFolderId = providedParentFolderId || rootFolderId;

    // If no explicit parent ID is provided but we have a path, try to find the folder
    if (!providedParentFolderId && parentPath && parentPath !== "Private ") {
      // Split the path and remove the first element (which is "Private ")
      const pathParts = parentPath.split('/').filter(part => part && part !== "Private ");
      
      if (pathParts.length > 0) {
        // Navigate the folder structure to find the correct parent folder ID
        let currentParentId = rootFolderId;
        
        for (const part of pathParts) {
          const folderQuery = await drive.files.list({
            q: `name='${part}' and mimeType='application/vnd.google-apps.folder' and '${currentParentId}' in parents and trashed=false`,
            fields: "files(id)",
          });
          
          if (folderQuery.data.files.length === 0) {
            return NextResponse.json({ 
              success: false, 
              error: `Parent folder path not found: ${part}` 
            });
          }
          
          currentParentId = folderQuery.data.files[0].id;
        }
        
        parentFolderId = currentParentId;
      }
    }

    // Create the folder in Google Drive
    const folderMetadata = {
      name: folderName,
      mimeType: 'application/vnd.google-apps.folder',
      parents: [parentFolderId],
    };

    const folderResponse = await drive.files.create({
      requestBody: folderMetadata,
      fields: 'id,name,mimeType,createdTime',
    });

    // Set permissions to make the folder accessible
    await drive.permissions.create({
      fileId: folderResponse.data.id,
      requestBody: {
        role: 'reader',
        type: 'anyone'
      }
    });

    return NextResponse.json({
      success: true,
      folder: {
        id: folderResponse.data.id,
        name: folderResponse.data.name,
        type: 'folder',
        parent: parentPath || 'Private ',
        createdAt: folderResponse.data.createdTime,
      }
    });
  } catch (error) {
    console.error("Error creating folder:", error);
    return NextResponse.json({ 
      success: false, 
      error: error.message 
    }, { status: 500 });
  }
}